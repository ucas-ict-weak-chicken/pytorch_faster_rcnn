...
def _smooth_l1_loss(bbox_pred, bbox_targets, bbox_inside_weights, bbox_outside_weights, sigma=1.0, dim=[1]):
    """
    Smooth L1 Loss in fast RCNN

    Args:
        bbox_pred(torch.Tensor): [B, A, H, W]
        bbox_targets(torch.Tensor): [B, A, H, W]
        bbox_inside_weights(torch.Tensor): [B, A, H, W], average mini batch
        bbox_outside_weights(torch.Tensor): 1
        sigma(float): sigma of smooth l1
        dim(list):  dims

    Returns:
        torch.Tensor: [B, 1], loss
    """
    sigma_2 = sigma ** 2
